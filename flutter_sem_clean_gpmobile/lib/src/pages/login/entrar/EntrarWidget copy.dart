...
//VARIAVEIS
  ...
  void initState() {
    ...
    HomeBloc().getUsuario().then((map) async {
      getUser = map;
    });
    ...
    return Container(
      child: Row(
        ...
        children: <Widget>[
          ...
          InkWell(
            onTap: () async {
              if (usuarioCtrl.text == null || usuarioCtrl.text == "") {
                await AlertDialogTemplate().showAlertDialogSimples(
                    context,
                    "Atencão",
                    "Para recuperar senha, necessario insirir um usuario!");
                // "Para recuperar senha, necessario primeiro login!");
              } else if (userSalvo == false) {
                await AlertDialogTemplate().showAlertDialogSimples(
                    context,
                    "Atencão",
                    "Para recuperar senha, necessario salvar usuario!");
              } else if (userSalvo == true) {
                if (usuarioCtrl.text == getUser) {
                  switch (plataforma) {
                    case "mob":
                  ...
                  await AlertDialogTemplate().showAlertDialogSimples(
                      context,
                      "Atencão",
                      "Para recuperar senha, necessário insirir um usuário válido!");
                }
              }
            },
            child: Text(
