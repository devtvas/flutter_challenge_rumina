...
import 'package:gpmobile/src/pages/home/HomeBloc.dart';
...
//VARIAVEIS
  ...
  void initState() {
    ...
    Timer(
      Duration(seconds: 1),
      () => setState(() {
        HomeBloc().getTelefoneColaborador().then((map) async {
          String _userTelefone = map;
          _telefoneOculto = _userTelefone.replaceRange(0, 5, '****-');
        });
        HomeBloc().getEmailColaborador().then((map) async {
          _userEmail = map;
          String left = _userEmail.split("@")[0];
          var right = _userEmail.split("@")[1];
          var tamanho = left.length;
          var metade = (tamanho / 2).round().toInt();
          var subString = left.substring(0, metade);
          _emailOculto = '$subString*****@$right';
          // _emailOculto = _userEmail.replaceRange(5, 4, '*****');
        });
      }),
    );
...
//   }
  ...
  Widget txtIntro(BuildContext context) {
    String h1 = 'Bem-vindo';
    String h2 = 'Escolha como recuperar senha!';
    return Padding(
              ...
              text: TextSpan(
                ...
                children: <TextSpan>[
                  TextSpan(
                    ...
                    // children: <TextSpan>[TextSpan(text: h2)],
                    // children: <TextSpan>[TextSpan(text: h2)],
                  ),
                ],
              ),
  ...
  _sendRecuperacao(String tipo, int intNotificar) {
    ...
    RecuperarSenhaBloc()
        .blocRecPorEmail(context, intNotificar, _acaoRecuperSenha, tipo, true);
  }
